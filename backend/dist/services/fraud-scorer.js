...
class FraudScorer {
     ...
     */
    async scoreEvidence(evidenceId, jobId, fileHash, userId, contentType) {
        const indicators = [];
     ...
     */
    async checkCrossJobEvidenceReuse(fileHash, userId, currentJobId) {
     ...
     */
    async checkDuplicateEvidenceWithinJob(fileHash, jobId) {
        const query = await this.db
            .collection('evidence')
            .where('fileHash', '==', fileHash)
            .where('jobId', '==', jobId)
     ...
     */
    async checkUserFraudHistory(userId) {
     ...
     */
    async checkTimingAnomalies(jobId, userId) {
     ...
     */
    async checkUserVelocity(userId) {
        const oneHourAgo = Date.now() - 60 * 60 * 1000;
        const recentSubmissions = await this.db
            .collection('evidence')
            .where('uploadedBy', '==', userId)
            .where('uploadedAt', '>=', firestore_1.Timestamp.fromMillis(oneHourAgo))
            .get();
    ...
    async getFraudStats() {
        ...
        let clean = 0, suspicious = 0, flagged = 0;
        let totalRisk = 0;
        query.forEach((doc) => {
...
exports.FraudScorer = FraudScorer;
