...
const VerificationOutputSchema = z.object({
  ...
  requirementsMet: z.record(z.boolean()),
  issues: z.array(z.string()),
  suggestedAction: z.enum(['release', 'refund', 'dispute']),
  analysis: z.string(),
});
 ...
 */
export class AIVerificationServiceEnhanced {
  private openai: OpenAI;
  private readonly AUTO_APPROVE_THRESHOLD = 0.90;
  private readonly MANUAL_REVIEW_THRESHOLD = 0.70;

  constructor(
    private db: Firestore,
    apiKey: string,
    private confidenceThreshold: number = 0.90
  ) {
   ...
   */
  private async fetchEvidenceContent(storageUrl: string, contentType: string): Promise<string | null> {
    const maxRetries = 3;
   ...
   */
  private async logFetchFailure(storageUrl: string, error: unknown): Promise<void> {
    try {
   ...
   */
  private async analyzeEvidenceWithGPT4o(
        ...
        if (['image/jpeg', 'image/png', 'image/gif', 'image/webp'].includes(e.contentType)) {
          ...
          if (imageData) {
            contentParts.push({
              ...
              image: {
                data: imageData,
                media_type: e.contentType as 'image/jpeg' | 'image/png' | 'image/gif' | 'image/webp',
              },
            });
          }
   ...
   */
  private parseAndValidateAIResponse(output: VerificationOutput, request: VerificationRequest): AIVerificationResult {
    try {
      ...
      return {
        jobId: request.jobId,
        verdict: verdict as 'approved' | 'rejected' | 'needs_review',
        confidence,
        completionScore,
