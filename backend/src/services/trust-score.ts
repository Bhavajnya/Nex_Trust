...
export class TrustScoreService {
  ...
  async calculateTrustScore(userId: string): Promise<TrustScoreData> {
    try {
      ...
      if (evidenceSnapshot.size > 0) {
        const approvedCount = evidenceSnapshot.docs.filter(
          (doc) => doc.data().verificationStatus === 'approved'
        ).length;
      ...
      const trustData: TrustScoreData = {
        userId,
        score,
        jobsCompleted,
        jobsAccepted,
   ...
   */
  async getLeaderboard(limit: number = 10): Promise<TrustScoreData[]> {
    try {
      ...
      return snapshot.docs.map((doc) => doc.data() as TrustScoreData);
    } catch (err) {
