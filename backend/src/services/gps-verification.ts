...
export class GPSVerificationService {
   ...
   */
  static isLocationRealistic(lat: number, lon: number): boolean {
   ...
   */
  static isAccuracyReasonable(accuracy?: number): boolean {
    // Accuracy > 100 meters is suspicious for evidence
    // Most phones can achieve 5-20 meter accuracy
    if (!accuracy) return true; // If not provided, assume ok
    return accuracy <= 100;
  }
   ...
   */
  static detectImpossibleLocationJump(
    loc1: LocationData,
    loc2: LocationData
  ): boolean {
   ...
   */
  static generateLocationFraudScore(
    location: LocationData,
    previousLocations?: LocationData[]
  ...
  /**
   * Verify evidence location is within allowed radius of job location
   * Returns { withinAllowedRadius, distanceMeters, radius }
   */
  static verifyLocationWithinRadius(
    jobLatitude: number,
    jobLongitude: number,
    evidenceLatitude: number,
    evidenceLongitude: number,
    allowedRadiusKm: number = 1 // Default 1km
  ): {
    ...
    const withinAllowedRadius = distanceMeters <= allowedRadiusMeters;
  ...
  /**
   * Combine GPS validation with location radius check
   * Used during evidence verification
   */
  static verifyEvidenceLocation(params: {
    jobLatitude: number;
    jobLongitude: number;
    evidenceLatitude: number;
    evidenceLongitude: number;
    ...
    // 1. Validate evidence coordinates
    if (!this.validateCoordinates(evidenceLatitude, evidenceLongitude)) {
      issues.push('Evidence coordinates out of valid range');
      confidence -= 0.3;
    }

    // 2. Validate job location coordinates
    if (!this.validateCoordinates(jobLatitude, jobLongitude)) {
      issues.push('Job location coordinates invalid');
      confidence -= 0.3;
    }

    // 3. Check GPS accuracy
    if (!this.isAccuracyReasonable(evidenceAccuracy)) {
      issues.push(`Low GPS accuracy: ${evidenceAccuracy}m`);
      confidence -= 0.2;
    }

    // 4. Check distance to job location
    const distanceResult = this.verifyLocationWithinRadius(
      jobLatitude,
      jobLongitude,
      evidenceLatitude,
      evidenceLongitude,
    ...
    if (!distanceResult.withinAllowedRadius) {
      issues.push(
        `Evidence location ${distanceResult.distanceKm.toFixed(2)}km away from job (allowed: ${allowedRadiusKm}km)`
      );
      confidence -= 0.4;
    }
    ...
    return {
      isValid: confidence >= 0.6 && distanceResult.withinAllowedRadius,
      withinRadius: distanceResult.withinAllowedRadius,
